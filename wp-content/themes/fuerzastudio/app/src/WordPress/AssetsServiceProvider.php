...
class AssetsServiceProvider implements ServiceProviderInterface {
	...
	public function bootstrap( $container ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
		...
		add_action( 'wp_enqueue_scripts', [ $this, 'enqueueStyleguideAssets' ] );
	...
	/**
	 * Enqueue styleguide assets.
	 *
	 * @return void
	 */
	public function enqueueStyleguideAssets() {
		// Enqueue only if we're in the styleguide template.
		if ( ! is_page_template( 'templates/styleguide.php' ) ) {
			return;
		}

		// Enqueue scripts.
		\Fuerza::core()->assets()->enqueueScript(
			'styleguide-js-bundle',
			\Fuerza::core()->assets()->getBundleUrl( 'styleguide', '.js' ),
			[ 'jquery' ],
			true
		);

		// Enqueue styles.
		$style = \Fuerza::core()->assets()->getBundleUrl( 'styleguide', '.css' );

		if ( $style ) {
			\Fuerza::core()->assets()->enqueueStyle(
				'styleguide-css-bundle',
				$style
			);
		}
	}
